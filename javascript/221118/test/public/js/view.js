...
const beforeUrl = document.referrer; // 이전 페이지 url
// console.log(beforeUrl.split("/").pop());

const plusHit = (beforeUrl) => {
  if (beforeUrl.split("/").pop() === "list.html") {
    board.hit++;
    const hitUpdateStr = JSON.stringify(boardsObj);
    localStorage.setItem("boards", hitUpdateStr);
  }
};
plusHit(beforeUrl);
